...
export default function SearchScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `/api/products/search?page=${page}&query=${query}&category=${category}&price=${price}&rating=${rating}&order=${order}&country=${country}`
        );
    ...
    fetchData();
  }, [category, country, error, order, page, price, query, rating]);

  // State for categories and countries
  ...
  const getFilterUrl = (filter, skipPathname) => {
    ...
    return `${
      skipPathname ? "" : "/search?"
    }category=${filterCategory}&country=${filterCountry}&query=${filterQuery}&price=${filterPrice}&rating=${filterRating}&order=${sortOrder}&page=${filterPage}`;
  };
  return (
